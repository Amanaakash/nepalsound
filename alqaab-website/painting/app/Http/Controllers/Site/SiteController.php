    ...
    public function gallery($id)
{
    ...
    // If album does not exist, show 404
    if (!$data['album_title']) {
        abort(404);
    }

    $data['gallery'] = Photos::where('status', 1)
        ->where('album_id', $id)
        ->orderBy('id', 'desc')
        ->get();
    ...
    $data['album_list'] = Album::where('status', 1)
        ->orderBy('id', 'desc')
        ->get();

    $data['nextPost'] = Album::where(
        'created_at',
        '>',
        $data['album_title']->created_at
    )->orderBy('created_at', 'asc')->first();

    $data['prevPost'] = Album::where(
        'created_at',
        '<',
        $data['album_title']->created_at
    ...
    return view(
        parent::loadView($this->view_path . '.gallery'),
        compact('data')
    );
}
//public function gallery($id)
