        ...
        <div class="container">
            @if(isset($all_view['setting']->logo))
                <a href="{{route('site.index')}}">
                    <img class="logo-part" src="{{ asset(\App\Support\SitePresentation::logoPath($all_view['setting']->logo)) }}" alt="Logo">
                </a>
            @endif
                    ...
                    @if(isset($data['menu']))
                        @foreach($data['menu'] as $row)
                            <li class="nav-item dropdown">
                                <a class="nav-link text-white {{ isset($row['child']) ? 'dropdown-toggle' : '' }}"
                                    href="{{ url($row['url']) }}" 
                                    id="navbarDropdown{{ $row['id'] ?? '' }}" 
                                    role="button"
                                    @if (isset($row['child'])) 
                                        data-bs-toggle="dropdown" 
                                        aria-expanded="false" 
                                    @endif
                                    @if(isset($row['target']) && $row['target'] == '_blank')
                                        target="_blank"
                                    @endif>
                                    {{ $row['menu_name'] }}
                                </a>
                                ...
                                @if (isset($row['child']) && count($row['child']) > 0)
                                    <ul class="dropdown-menu" aria-labelledby="navbarDropdown{{ $row['id'] ?? '' }}">
                                        @foreach ($row['child'] as $child)
                                            <li>
                                                <a class="dropdown-item" 
                                                   href="{{ url($child['url']) }}"
                                                   @if(isset($child['target']) && $child['target'] == '_blank')
                                                        target="_blank"
                                                   @endif>
                                                    {{ $child['menu_name'] }}
                                                </a>
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="mobileMenu" aria-labelledby="mobileMenuLabel">
    <div class="offcanvas-header" style="background-color: #000000">
        <h5 class="offcanvas-title" id="mobileMenuLabel">
            @if(isset($all_view['setting']->logo))
                <a href="{{route('site.index')}}">
                    <img src="{{ asset(\App\Support\SitePresentation::logoPath($all_view['setting']->logo)) }}" alt="Logo" style="height: 60px; width: 90px;">
                </a>
            @endif
        </h5>
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-white {{ isset($row['child']) ? 'dropdown-toggle' : '' }}"
                            ...
                            @if (isset($row['child'])) 
                                ...
                                data-bs-target="#submenu{{ $row['id'] ?? '' }}" 
                            @endif
                            style="border-bottom: 2px solid #797979;">
                            {{ $row['menu_name'] }}
                        </a>

                        @if(isset($row['child']) && count($row['child']) > 0)
                            <ul class="collapse list-unstyled ps-3" id="submenu{{ $row['id'] ?? '' }}">
                                @foreach($row['child'] as $child)
                                    <li>
                                        <a class="nav-link text-white" href="{{ url($child['url']) }}">
                                            {{ $child['menu_name'] }}
                                        </a>
                                    </li>
                                @endforeach
...
.offcanvas-body .list-unstyled .nav-link {
    padding-left: 1rem;
    font-size: 0.9rem;
